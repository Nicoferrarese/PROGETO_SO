...
int delete(tabella* tabella,int num,int id){
    int ok=1;
    nodo* element=(nodo*)malloc(sizeof(nodo));
        element->next=NULL;
        element->prev=NULL;
    element=tabella->um_number[num];
    do{
    if(element->el !=NULL){
        if(element->el->diritto == id){
            //rm umbrella
            if(element->prev==NULL){
                if(element->next==NULL){
                    printf("Rimozione Unico elemento\n");
                    tabella->um_number[num]->el=NULL;
                    free(element);
                    return 1;
                }
                if(element->next!=NULL){
                    printf("Rimozione Primo elemento");
                    element->next->prev=NULL;
                    tabella->um_number[num]=element->next;
                    return 1;
                }
            }
            if(element->prev!=NULL){
                if(element->next==NULL){
                    printf("Rimozione Ultimo elemento");
                    element->prev->next=NULL;
                    free(element);
                    return 1;
                }
                if(element->next!=NULL){
                    printf("Rimozione elemento in mezzo");
        ...
        else if(element->next==NULL)ok=0;
        }
    }while(ok);
